...
TEST_SHORT = "https://t.co/4jLPOtjKtJ"
...
async def main():
    ...
    try:
        r = await google_safe_browsing(TEST_URL)
    ...
    print("\n4. PhishTank")
    try:
        r = await phishtank_check(TEST_URL)
    ...
    print("\n5. OpenPhish")
    try:
        r = await openphish_check(TEST_URL)
    ...
    print("\n6. AbuseIPDB")
    try:
        r = await abuseipdb_check(TEST_URL)
    ...
    print("\n7. URL Unshortening")
    try:
        final = await unshorten_url(TEST_SHORT)
        short = is_shortener(TEST_SHORT)
